    ...
    match
      ...
      return
        (forall (B3 : y -> Type) (p3 : ∑ x : y, B3 x),
         (forall p : Pack A1 y, B1 (ProjT1 p) ≅ B3 (ProjT2 p)) -> p1 ≅ p3 -> pi1 p1 ≅ pi1 p3)
    with
    | eq_refl =>
        ...
        match
          ...
          return
            (forall p4 : ∑ x : A1, y x,
             (forall p : Pack A1 A1, B1 (ProjT1 p) ≅ y (ProjT2 p)) -> p1 ≅ p4 -> pi1 p1 ≅ pi1 p4)
        with
        | eq_refl =>
            ...
            | @heqPair _ _ _ _ pT0 pp =>
                let pp0 : transport eq_refl p1 = p4 :=
                  Logic.eq_ind pT0
                    ...
                    eq_refl (K pT0) in
                match pp0 in (_ = y) return (pi1 p1 ≅ pi1 y) with
                | eq_refl => heq_refl (pi1 p1)
                end
            end
        end p3 hB0 hp0
    end B2 p2 hB hp
     ...
     : forall (A1 A2 : Type) (B1 : A1 -> Type) (B2 : A2 -> Type) (p1 : ∑ x : A1, B1 x)
         (p2 : ∑ x : A2, B2 x),
       A1 ≅ A2 -> (forall p : Pack A1 A2, B1 (ProjT1 p) ≅ B2 (ProjT2 p)) -> p1 ≅ p2 -> pi1 p1 ≅ pi1 p2.
